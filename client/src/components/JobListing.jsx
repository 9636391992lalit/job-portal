  ...
  return (
    <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
          ...
          isSearched && (searchFilter.title !== "" || searchFilter.location != "") && (
            <>
              ...
              <div className='mb-4 text-gray-600'>
                {searchFilter.title && (
                  <span className='inline-flex items-center gap-2.5 bg-blue-50 border-blue-200 py-1.5 px-4  rounded'>
                    {searchFilter.title}
                    <img onClick={() => setSearchFilter(prev => ({ ...prev, title: "" }))} className='cursor-pointer' src={assets.cross_icon} alt="" />
                  </span>
                )}
                {searchFilter.location && (
                  <span className='ml-2 inline-flex items-center gap-2.5 bg-red-50 border-red-200 py-1.5 px-4 rounded'>
                    {searchFilter.location}
          ...
          <ul className='space-y-4 text-gray-600'>
            {
              JobLocations.map((location, index) => (
                <li className='flex gap-3 items-ceter ' key={index}>
      ...
      <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4 '>
        ...
        <p className='mb-8'>Get Your Desired Jobs from top of the Companies....</p>
        ...
        {filterdJobs.length > 0 && (
          <div className='flex items-center justify-center mt-10 space-x-2'>
            ...
            <a href="#job-list">
              <img onClick={() => setCurrentPage(Math.min(currentPage + 1, Math.ceil(filterdJobs.length / 6)))} src={assets.right_arrow_icon} alt="" />
            </a>
