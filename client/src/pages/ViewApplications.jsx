...
const ViewApplications = () => {
  ...
  // Function to comment job Application Status
  ...
  return applicants ? applicants.length === 0 ? (<div className='flex items-center justify-center h-[70vh]'>
   <p className='text-xl sm:text-2xl '>No Application is Avialable</p>
  </div>) : (
            ...
            {applicants.filter(item => item.jobId && item.userId).map((applicant, index) =>
              <tr key={index} className='text-gray-700'>
                ...
                <td className='py-2 px-4 border-b'>
                  <a href={applicant.userId.resume} target="blank" className='=bg-blue-50 text-blue-400 px-3 py-1 rounded inline-flex gap-2 items-center'>
